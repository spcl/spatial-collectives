...
for vec_len in {0..12}
  ...
  cslc layout_2d_pre_order_test.csl --fabric-dims=519,514 \
  --fabric-offsets=4,1 --params=Nx_start:$((2**$vec_len)),Pw:512,Ph:512,Algo:0,is_allred:0,step:$int_result -o out --memcpy --channels=1
...
do
  ...
  int_result=${sqrt_result%.*}
  cslc layout_2d_pre_order_test.csl --fabric-dims=519,514 \
  --fabric-offsets=4,1 --params=Nx_start:256,Pw:$((2**$log_pes)),Ph:$((2**$log_pes)),Algo:0,is_allred:0,step:$int_result -o out --memcpy --channels=1
  cs_python run_2d_specific_pe_test.py --name out
done